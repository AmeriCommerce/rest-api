...
module AmeriCommerce
	...
	class ApiClient
		...
		def get_token(username, api_key)
			temp = start_negotiation(username, api_key)
			token_data = verify(temp['auth_id'], temp['code'])
	...
	private
		...
		def start_negotiation(username, api_key)
			sig = create_signature(@app_secret, username, api_key, @app_id, @app_scope, @store_domain)
			data = {
				:app_id => @app_id,
				:scope => @app_scope,
				:redirect_url => @store_domain,
				:username => username,
				:signature => sig
			}.to_json
			headers = {
			...
			uri = URI("https://#{@store_domain}/api/oauth")
			http = Net::HTTP.new(uri.host, uri.port)
			http.use_ssl = true
			# if using a development cert
			# http.verify_mode = OpenSSL::SSL::VERIFY_NONE
		...
		def verify(auth_id, code)
			sig = create_signature(@app_secret, code, @app_id, @app_scope, @store_domain)
			data = {
				:app_id => @app_id,
				:auth_id => auth_id,
				:signature => sig
